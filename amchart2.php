    ...
    $sql="select time_index,sumtime,median from
        ...
        (SELECT
         ...
         FROM
             ...
             FROM RFID_food, (SELECT @rnk:=0, @pre:=null)init
             ORDER by time_index, sumtime)e1 
             JOIN 
             (SELECT time_index, count(*) cnt FROM RFID_food GROUP by time_index) e2
             using(time_index)
        WHERE e1.rnk in (cnt/2+0.5, cnt/2, cnt/2+1))  a
        group by time_index) b

        left JOIN RFID_food
        on b.time_index2 = RFID_food. time_index
...
createSeries("sumtime", "進食次數");
createSeriesdash("median", "進食次數中位數");
